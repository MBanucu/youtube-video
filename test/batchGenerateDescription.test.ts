...
test(
  ...
  async () => {
    ...
    expect(
      existsSync(enPath),
      `Expected EN description file at ${enPath}`,
    ).toBe(true)
    expect(
      existsSync(dePath),
      `Expected DE description file at ${dePath}`,
    ).toBe(true)

    // Check content (real AI-generated)
    const enContent = await Bun.file(
      join(tmpDescDir, 'test-description_en.txt'),
    ).text()
    ...
    const deContent = await Bun.file(
      join(tmpDescDir, 'test-description_de.txt'),
    ).text()
